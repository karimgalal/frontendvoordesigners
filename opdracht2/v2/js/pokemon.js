...
function computerAanval() {
    ...
    if (damage >= leven[0]) {
        ...
        document.querySelectorAll('h1')[2].textContent = "You lost.. Better luck next time! Press X to play again.";
    } else {
        leven[0] -= damage;
...
function spelerAanval() {
    ...
    if (damage >= leven[1]) {
        ...
        document.querySelectorAll('h1')[2].textContent = "Great job, you won! Press X to play again.";
    } else {
        leven[1] -= damage;
...
document.addEventListener('keypress', function(event)  {
    ...
    if (event.code == 'KeyQ' && aanvalluh == true) {
        spelerAanval();
    }

    if (event.code == 'KeyW' && aanvalluh == true) {
        spelerAanval();
    }

    if (event.code == 'KeyE' && aanvalluh == true) {
        spelerAanval();
    }

    if (event.code == 'KeyR' && aanvalluh == true) {
        spelerAanval();
    }
